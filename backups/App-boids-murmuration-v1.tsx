...
// NATURAL BOIDS MURMURATION - Organic flocking behavior
  ...
  useEffect(() => {
    ...
    class Boid {
      ...
      constructor() {
        ...
        this.velocity = Vector.random().mult(1.5 + Math.random() * 1);
        this.acceleration = new Vector(0, 0);
        this.maxSpeed = 2.2 + Math.random() * 0.8; // Varied speeds
        this.maxForce = 0.03 + Math.random() * 0.02; // Varied agility
        
        // Visual properties
        this.size = Math.random() * 3 + 2.5; // 2.5-5.5px birds
        this.wingPhase = Math.random() * Math.PI * 2;
        this.wingSpeed = 0.08 + Math.random() * 0.04;
        this.alpha = 0.7 + Math.random() * 0.3;
        this.trail = [];
        this.maxTrailLength = 6 + Math.floor(Math.random() * 4);
        this.isLeader = Math.random() < 0.08; // 8% leaders
      }
      ...
      edges() {
        ...
        if (this.position.x > canvasWidth + 20) this.position.x = -20;
        if (this.position.x < -20) this.position.x = canvasWidth + 20;
        if (this.position.y > canvasHeight + 20) this.position.y = -20;
        if (this.position.y < -20) this.position.y = canvasHeight + 20;
      }

      align(boids: Boid[]): Vector {
        const perception = this.isLeader ? 60 : 45; // Leaders see further
      ...
      cohesion(boids: Boid[]): Vector {
        const perception = this.isLeader ? 70 : 50;
        let steering = new Vector(0, 0);
        let total = 0;
      ...
      separation(boids: Boid[]): Vector {
        const perception = 25; // Consistent personal space
        let steering = new Vector(0, 0);
        let total = 0;
      ...
      // Add subtle center attraction to keep flock on screen
      centerAttraction(): Vector {
        const canvasWidth = canvas?.width || 0;
        const canvasHeight = canvas?.height || 0;
        const center = new Vector(canvasWidth * 0.5, canvasHeight * 0.5);
        const distance = this.position.dist(center);
        
        let steering = new Vector(0, 0);
        
        // Only attract if far from center
        if (distance > Math.min(canvasWidth, canvasHeight) * 0.3) {
          steering = center.clone().sub(this.position);
          steering.normalize().mult(this.maxSpeed);
          steering.sub(this.velocity);
          steering.limit(this.maxForce * 0.3); // Weak attraction
        }
      ...
      flock(boids: Boid[]) {
        ...
        const centerPull = this.centerAttraction();

        // Natural weighting for realistic murmuration
        alignment.mult(1.0);
        cohesion.mult(1.2);
        separation.mult(1.8); // Strongest - avoid collisions
        centerPull.mult(0.5);
      ...
      update() {
        // Update trail
        this.trail.unshift({x: this.position.x, y: this.position.y, alpha: this.alpha});
        ...
        this.trail.forEach((point, i) => {
          point.alpha = (this.alpha * (this.maxTrailLength - i)) / this.maxTrailLength * 0.25;
        });
      ...
      draw() {
        ...
        this.trail.forEach((point, i) => {
          if (i > 0) {
            ...
            ctx!.strokeStyle = 'rgba(47, 107, 255, 0.5)';
            ctx!.lineWidth = this.size * 0.08 * (1 - i / this.maxTrailLength);
            ctx!.beginPath();
        ...
        const speedFactor = Math.min(this.velocity.mag() / this.maxSpeed, 1);
        ...
        // Enhanced glow for leaders
        const glowIntensity = this.isLeader ? 1.3 : 0.9;
        ctx!.shadowColor = `rgba(47, 107, 255, ${0.3 * glowIntensity})`;
        ctx!.shadowBlur = this.size * 1.8;
        
        const birdColor = `rgba(47, 107, 255, ${0.85 + speedFactor * 0.15})`;
        const bodyLength = this.size * 2.2;
        
        // === BIRD BODY ===
        ctx!.fillStyle = birdColor;
        ctx!.beginPath();
        ctx!.ellipse(0, 0, bodyLength * 0.7, this.size * 0.35, 0, 0, Math.PI * 2);
        ctx!.fill();
        ...
        ctx!.lineTo(bodyLength * 0.95, -this.size * 0.12);
        ctx!.lineTo(bodyLength * 0.95, this.size * 0.12);
        ctx!.closePath();
        ctx!.fill();
        ...
        ctx!.lineWidth = this.size * 0.12 * (1 + speedFactor * 0.2);
        ctx!.lineCap = 'round';
        
        const wingLength = this.size * 2.4 * (1 + speedFactor * 0.4);
        const wingCurve = this.size * (1.1 + speedFactor * 0.6);
        const wingTip = this.size * (0.7 + speedFactor * 0.3);
        const wingFlexion = wingFlap * (0.25 + speedFactor * 0.15);
        
        // Wing curves
        ctx!.beginPath();
        ctx!.moveTo(-bodyLength * 0.15, -this.size * 0.15);
        ...
        if (this.size > 3) {
          ...
          const tailLength = this.size * 1.0;
          const tailSpread = this.size * 0.4;
          
          // Center tail
          ctx!.beginPath();
          ctx!.moveTo(-bodyLength * 0.7, 0);
          ctx!.lineTo(-bodyLength * 0.7 - tailLength, 0);
          ctx!.stroke();
          
          // Side tails
          ctx!.beginPath();
          ctx!.moveTo(-bodyLength * 0.6, -this.size * 0.1);
          ctx!.lineTo(-bodyLength * 0.6 - tailLength * 0.7, -tailSpread);
          ctx!.stroke();
    ...
    const init = () => {
      flock = [];
      const numberOfBirds = Math.min(120, Math.max(80, Math.floor((canvas?.width || 0) / 12))); // 80-120 birds
