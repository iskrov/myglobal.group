...
// DENSE MURMURATION WITH BOLD SIMULTANEOUS M & G FORMATIONS
const FlockAnimation: React.FC = () => {
  ...
  useEffect(() => {
    ...
    class Bird {
      ...
      letterType: string | null; // 'M' or 'G'

      constructor(x: number, y: number) {
        this.position = new Vector(x, y);
        this.velocity = Vector.random().mult(2);
        this.acceleration = new Vector(0, 0);
        this.maxSpeed = 3.0; // Faster for snappier formations
        this.maxForce = 0.05; // Stronger force for tighter letters
        
        // Visual properties - smaller for more definition
        this.size = 0.8 + Math.random() * 0.4; // 0.8-1.2px - tiny for clarity
      ...
      align(birds: Bird[]): Vector {
        ...
        for (let other of birds) {
          ...
          if (other !== this && d < perception) {
            // Only align with birds going to same letter
      ...
      // Move towards letter formation target with strong attraction
      seekTarget(): Vector {
        ...
        // Strong arrival behavior for crisp letters
        if (d < 5) {
        ...
        steer.limit(this.maxForce * 3); // Very strong force for tight formations
        return steer;
      }
      ...
      flock(birds: Bird[]): void {
        ...
        // Adjust weights based on formation phase
        if (currentPhase === 'forming' || currentPhase === 'holding') {
          alignment.mult(0.2);
          cohesion.mult(0.3);
          separation.mult(1.0);
          target.mult(5 * formationStrength); // VERY strong when forming
      ...
      draw(): void {
        ...
        ctx!.fillStyle = 'rgba(47, 127, 255, 0.9)';
        ctx!.beginPath();
    ...
    // Get M formation position - LEFT SIDE
    const getLetterMPosition = (index: number, total: number): Vector => {
      const canvasWidth = canvas?.width || 0;
      const canvasHeight = canvas?.height || 0;
      const centerX = canvasWidth * 0.25; // Left quarter
      const centerY = canvasHeight * 0.5;
      const letterHeight = 150; // Bigger letters
      const letterWidth = 120;
      
      const progress = index / Math.max(1, total - 1);
      
      if (progress <= 0.25) {
        // Left vertical - thick stroke
        const t = progress * 4;
        const xOffset = (Math.random() - 0.5) * 8; // Slight thickness variation
        return new Vector(
          centerX - letterWidth/2 + xOffset,
    ...
    // Get G formation position - RIGHT SIDE (PROVEN VERSION)
    const getLetterGPosition = (index: number, total: number): Vector => {
      const canvasWidth = canvas?.width || 0;
      const canvasHeight = canvas?.height || 0;
      const centerX = canvasWidth * 0.75; // Right three-quarters
      const centerY = canvasHeight * 0.5;
      const radius = 75; // Good size for visibility
      
      const arcBirdCount = Math.floor(total * 0.8); // 80% for the C-arc
      const arrowBirdCount = total - arcBirdCount; // 20% for the arrow
      
      if (index < arcBirdCount) {
        // C-shaped arc - our proven formation
        const progress = index / Math.max(1, arcBirdCount - 1);
        const startAngle = -Math.PI * 0.1;
        const arcLength = Math.PI * 1.6;
        const angle = startAngle - progress * arcLength; // Counter-clockwise
        
        // Add thickness variation for bold appearance
        ...
        // Left-pointing arrow (proven to work!)
        const arrowIndex = index - arcBirdCount;
        const arrowTipX = centerX + radius * 0.5; // Arrow tip position
        const arrowTipY = centerY;
        const arrowLength = -40; // Points left
        const arrowSpread = 35; // Wing spread
    ...
    const init = () => {
      ...
      // MANY MORE birds for bold letters!
      const numberOfBirds = Math.min(1200, Math.max(800, Math.floor(canvasWidth / 1.8)));
      
      console.log(`Creating dense murmuration with ${numberOfBirds} birds for bold letters`);
    ...
    const updatePhase = () => {
      ...
      // Phase timing: FAST first formation (2 seconds), then regular cycle
      if (currentPhase === 'flow') {
        // First formation happens quickly, then normal timing
        const isFirstFormation = time < 200; // Within first ~3 seconds of animation
        const flowDuration = isFirstFormation ? 100 : 300 + Math.random() * 120; // 1.7s first time, then 5-7s
        
        if (phaseTimer > flowDuration) {
          currentPhase = 'forming';
          phaseTimer = 0;
          
          // Assign BOTH M and G targets simultaneously!
      ...
      } else if (currentPhase === 'forming' && phaseTimer > 60) { // 1 second to form
        currentPhase = 'holding';
        phaseTimer = 0;
      } else if (currentPhase === 'holding' && phaseTimer > 60 + Math.random() * 60) { // 1-2 seconds hold
      ...
      } else if (currentPhase === 'dispersing' && phaseTimer > 60) { // 1 second to disperse
        currentPhase = 'flow';
        phaseTimer = 0;
      }
      
      // Update formation strength for smooth transitions
      if (currentPhase === 'forming') {
        formationStrength = Math.min(1, formationStrength + 0.05); // Quick formation
      } else if (currentPhase === 'holding') {
        formationStrength = 1; // Full strength hold
      } else if (currentPhase === 'dispersing') {
    ...
    const animate = () => {
      ...
      // Less fade for clearer letters
      ctx!.fillStyle = 'rgba(11, 21, 38, 0.04)';
