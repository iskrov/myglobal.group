...
const FlockAnimation: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Get G formation position - RIGHT SIDE
    const getLetterGPosition = (index: number, total: number): Vector => {
      const canvasWidth = canvas?.width || 0;
      const canvasHeight = canvas?.height || 0;
      const centerX = canvasWidth * 0.75; // Right three-quarters
      const centerY = canvasHeight * 0.5;
      const radius = 75; // Bigger G
      
      const arcBirdCount = Math.floor(total * 0.75); // 75% for arc
      const barBirdCount = Math.floor(total * 0.25); // 25% for bar
      
      if (index < arcBirdCount) {
        // C-shaped arc - thicker
        const progress = index / Math.max(1, arcBirdCount - 1);
        const startAngle = Math.PI * 0.3;
        const endAngle = -Math.PI * 0.3;
        const angleRange = (2 * Math.PI) - (startAngle - endAngle);
        ...
        const barProgress = (index - arcBirdCount) / Math.max(1, barBirdCount - 1);
        const yOffset = (Math.random() - 0.5) * 8;
        return new Vector(
          centerX + barProgress * radius * 0.8,
          centerY + yOffset
        );
      }
    };
    ...
    const updatePhase = () => {
      ...
      // Phase timing: flow (5-7s) -> forming (1s) -> holding (1-2s) -> dispersing (1s) -> flow
      if (currentPhase === 'flow' && phaseTimer > 300 + Math.random() * 120) { // 5-7 seconds
