...
// DENSE MURMURATION - Massive flocks with organic flowing shapes
const FlockAnimation: React.FC = () => {
  ...
  useEffect(() => {
    ...
    let flock: Boid[] = [];
    let time = 0;
    let currentAttractor: { x: number, y: number, type: string, strength: number } | null = null;
    ...
    class Boid {
      ...
      constructor() {
        ...
        this.maxSpeed = 1.8 + Math.random() * 1.2; // Varied speeds for natural flow
        this.maxForce = 0.02 + Math.random() * 0.02; 
        
        // Visual properties - much smaller birds for dense effect
        this.size = 0.8 + Math.random() * 0.7; // 0.8-1.5px - tiny birds!
        this.alpha = 0.6 + Math.random() * 0.4;
        this.isLeader = Math.random() < 0.05; // 5% leaders
        this.personalityFactor = 0.7 + Math.random() * 0.6;
      }
      ...
      edges() {
        ...
        // Soft boundaries - birds turn away from edges
        if (this.position.x < margin) {
          const force = new Vector(this.maxForce * 2, 0);
        ...
        if (this.position.y < margin) {
          const force = new Vector(0, this.maxForce * 2);
          this.acceleration.add(force);
        }
      ...
      align(boids: Boid[]): Vector {
        const perception = this.isLeader ? 40 : 25; // Tighter flocking
        let steering = new Vector(0, 0);
        let total = 0;
        
        for (let other of boids) {
          if (other !== this && this.position.dist(other.position) < perception) {
      ...
      cohesion(boids: Boid[]): Vector {
        const perception = this.isLeader ? 50 : 30; // Enhanced cohesion for dense flocks
        let steering = new Vector(0, 0);
        let total = 0;
      ...
      separation(boids: Boid[]): Vector {
        const perception = 15; // Tighter separation for density
        let steering = new Vector(0, 0);
        let total = 0;
      ...
      // Flow field for organic movement patterns
      flowField(): Vector {
        const canvasWidth = canvas?.width || 0;
        const canvasHeight = canvas?.height || 0;
        
        // Create flowing patterns using noise-like functions
        ...
        force.mult(this.maxForce * 0.3);
        
        return force;
      }

      // Attractor force for letter formations
      attractorForce(): Vector {
        if (!currentAttractor) return new Vector(0, 0);
        
        let target = new Vector(currentAttractor.x, currentAttractor.y);
        
        // Add letter-specific positioning
        if (currentAttractor.type === 'M') {
          target = this.getLetterMTarget();
        } else if (currentAttractor.type === 'G') {
          target = this.getLetterGTarget();
        }
        
        const distance = this.position.dist(target);
        if (distance < 200) { // Only attract nearby birds
          const force = target.clone().sub(this.position);
          force.normalize();
          force.mult(currentAttractor.strength * this.maxForce);
          // Fade with distance
          force.mult(1 - distance / 200);
          return force;
        }
      ...
      getLetterMTarget(): Vector {
        const canvasWidth = canvas?.width || 0;
        const canvasHeight = canvas?.height || 0;
        const centerX = currentAttractor!.x;
        const centerY = currentAttractor!.y;
        
        // Assign birds to different parts of M based on their position
        const relativeX = this.position.x - centerX;
        const letterWidth = 120;
        const letterHeight = 80;
        
        if (relativeX < -letterWidth/3) {
          // Left stroke
          return new Vector(centerX - letterWidth/2, centerY + Math.random() * letterHeight - letterHeight/2);
        } else if (relativeX < letterWidth/3) {
          // Middle V
          const progress = (relativeX + letterWidth/3) / (letterWidth * 2/3);
          const vY = Math.abs(progress - 0.5) * letterHeight;
          return new Vector(centerX + relativeX, centerY - letterHeight/2 + vY);
        } else {
          // Right stroke
          return new Vector(centerX + letterWidth/2, centerY + Math.random() * letterHeight - letterHeight/2);
        }
      ...
      getLetterGTarget(): Vector {
        const centerX = currentAttractor!.x;
        const centerY = currentAttractor!.y;
        const radius = 60;
        
        // Create G shape with C + horizontal bar
        const angle = Math.random() * Math.PI * 1.5 + Math.PI * 0.25; // 3/4 circle
        const x = centerX + Math.cos(angle) * radius;
        const y = centerY + Math.sin(angle) * radius;
        
        // Add horizontal bar
        if (Math.random() < 0.3) {
          return new Vector(centerX + Math.random() * radius * 0.8, centerY);
        }
      ...
      flock(boids: Boid[]) {
        ...
        const attractor = this.attractorForce();

        // Enhanced weighting for dense organic patterns
        alignment.mult(1.2);  // Stronger alignment for flowing movements
        cohesion.mult(2.0);   // Much stronger cohesion for dense clusters
        separation.mult(1.8); // Prevent overcrowding
        flow.mult(1.5);       // Strong flow field for organic shapes
        attractor.mult(0.8);  // Moderate letter formation

        this.acceleration.add(alignment);
        this.acceleration.add(cohesion);
        this.acceleration.add(separation);
        this.acceleration.add(flow);
        this.acceleration.add(attractor);
      }
      ...
      draw() {
        // Draw as simple filled circles for maximum density effect
        ctx!.save();
        ctx!.globalAlpha = this.alpha;
        
        // Leaders slightly larger and brighter
        if (this.isLeader) {
          ctx!.fillStyle = `rgba(47, 107, 255, 0.95)`;
          ctx!.beginPath();
          ctx!.arc(this.position.x, this.position.y, this.size * 1.5, 0, Math.PI * 2);
          ctx!.fill();
        } else {
          ctx!.fillStyle = `rgba(47, 107, 255, 0.8)`;
          ctx!.beginPath();
    ...
    const init = () => {
      flock = [];
      const canvasWidth = canvas?.width || 0;
      // MASSIVE bird count for proper murmuration density!
      const numberOfBirds = Math.min(800, Math.max(400, Math.floor(canvasWidth / 2))); // 400-800 birds!
      
      console.log(`Creating murmuration with ${numberOfBirds} birds`);
    ...
    const updateAttractor = () => {
      ...
      // Natural timing for letter formations
      if (attractorTimer > 800) { // ~13 seconds at 60fps
        const canvasWidth = canvas?.width || 0;
        const canvasHeight = canvas?.height || 0;
        
        if (!currentAttractor || Math.random() < 0.3) {
          // Create new attractor
          const types = ['flow', 'M', 'G'];
          const type = types[Math.floor(Math.random() * types.length)];
          
          if (type === 'flow') {
            currentAttractor = null; // Pure organic flow
          } else {
            currentAttractor = {
    ...
    const animate = () => {
      ...
      // Subtle fade for organic flow effect
      ctx!.fillStyle = 'rgba(11, 21, 38, 0.05)';
