...
// DENSE MURMURATION WITH COORDINATED LETTER FORMATIONS
const FlockAnimation: React.FC = () => {
  ...
  useEffect(() => {
    ...
    let flocks: { birds: Boid[], targetFormation: string | null, formationProgress: number }[] = [];
    let time = 0;
    let letterTimer = 0;
    ...
    class Boid {
      ...
      constructor(x: number, y: number, flockId: number) {
        this.position = new Vector(x, y);
        this.velocity = Vector.random().mult(1.2 + Math.random() * 0.8);
        this.acceleration = new Vector(0, 0);
        this.maxSpeed = 2.2 + Math.random() * 0.8;
        this.maxForce = 0.025 + Math.random() * 0.015;
        
        // Visual properties - small birds for density
        this.size = 1.0 + Math.random() * 0.8; // 1.0-1.8px
        this.alpha = 0.7 + Math.random() * 0.3;
        this.isLeader = Math.random() < 0.06; // 6% leaders
        this.flockId = flockId;
        this.targetPosition = null;
        this.personalityFactor = 0.8 + Math.random() * 0.4;
      }
      ...
      edges() {
        ...
        if (this.position.x < margin) {
          const force = new Vector(this.maxForce * 3, 0);
        ...
        if (this.position.y < margin) {
          const force = new Vector(0, this.maxForce * 3);
          this.acceleration.add(force);
        }
      ...
      align(boids: Boid[]): Vector {
        const perception = this.isLeader ? 35 : 25;
      ...
      cohesion(boids: Boid[]): Vector {
        const perception = this.isLeader ? 45 : 30;
        let steering = new Vector(0, 0);
        let total = 0;
      ...
      flowField(): Vector {
        ...
        const noise1 = Math.sin(this.position.x * noiseScale + timeScale) * 
                      Math.cos(this.position.y * noiseScale + timeScale * 0.7);
        const noise2 = Math.cos(this.position.x * noiseScale * 1.3 + timeScale * 1.1) * 
                      Math.sin(this.position.y * noiseScale * 0.8 + timeScale);
        
        const angle = noise1 * Math.PI + noise2 * Math.PI * 0.5;
        const force = new Vector(Math.cos(angle), Math.sin(angle));
        force.mult(this.maxForce * 0.5);
        
        return force;
      }

      // Move towards letter formation target
      formationForce(): Vector {
        if (!this.targetPosition) return new Vector(0, 0);
        
        const distance = this.position.dist(this.targetPosition);
        
        if (distance < 3) {
          // Close enough - gentle holding force
        ...
        // Strong force when far, gentle when close
        const strength = Math.min(distance / 100, 1) * this.maxForce * 3;
        force.mult(strength * this.personalityFactor);
        
        return force;
      }

      flock(boids: Boid[], formationStrength: number) {
        const alignment = this.align(boids);
        const cohesion = this.cohesion(boids);
        const separation = this.separation(boids);
        const flow = this.flowField();
        const formation = this.formationForce();

        // Dynamic weighting based on formation progress
        alignment.mult(1.0 * (1 - formationStrength * 0.5));
        cohesion.mult(1.8 * (1 - formationStrength * 0.3));
        separation.mult(2.0);
        flow.mult(0.8 * (1 - formationStrength));
        formation.mult(2.5 * formationStrength); // Strong when forming letters

        this.acceleration.add(alignment);
        this.acceleration.add(cohesion);
        this.acceleration.add(separation);
        this.acceleration.add(flow);
        this.acceleration.add(formation);
      }
      ...
      draw() {
        ...
        // Color based on flock
        const colors = [
          'rgba(47, 107, 255, 0.85)',  // Blue
          'rgba(0, 195, 137, 0.85)',   // Teal
          'rgba(255, 176, 32, 0.85)',  // Amber
          'rgba(229, 75, 75, 0.85)'    // Red
        ];
        
        const color = colors[this.flockId % colors.length];
        
        if (this.isLeader) {
          ctx!.fillStyle = color.replace('0.85', '0.95');
          ctx!.beginPath();
          ctx!.arc(this.position.x, this.position.y, this.size * 1.4, 0, Math.PI * 2);
          ctx!.fill();
        } else {
          ctx!.fillStyle = color;
          ctx!.beginPath();
    ...
    // Get position for M formation
    const getLetterMPosition = (index: number, total: number, centerX: number, centerY: number) => {
    ...
    // Get position for G formation
    const getLetterGPosition = (index: number, total: number, centerX: number, centerY: number) => {
      const radius = 60;
      const progress = index / Math.max(1, total - 1);
      
      if (progress <= 0.75) {
        // C-shaped arc (3/4 circle)
        const angle = -Math.PI * 0.25 + progress * Math.PI * 2;
        return new Vector(
    ...
    const init = () => {
      ...
      // Create 3-4 distinct flocks
      const numberOfFlocks = 3 + Math.floor(Math.random() * 2);
      const totalBirds = Math.min(800, Math.max(500, Math.floor(canvasWidth / 2.5)));
      const birdsPerFlock = Math.floor(totalBirds / numberOfFlocks);
      
      console.log(`Creating ${numberOfFlocks} flocks with ~${birdsPerFlock} birds each (${totalBirds} total)`);
      
      for (let f = 0; f < numberOfFlocks; f++) {
        const flock: Boid[] = [];
        
        // Start each flock in a clustered area
        const flockCenterX = canvasWidth * (0.2 + (f / numberOfFlocks) * 0.6);
        const flockCenterY = canvasHeight * (0.3 + Math.random() * 0.4);
        const spread = 80;
        
        for (let i = 0; i < birdsPerFlock; i++) {
          const angle = Math.random() * Math.PI * 2;
          const distance = Math.random() * spread;
          const x = flockCenterX + Math.cos(angle) * distance;
          const y = flockCenterY + Math.sin(angle) * distance;
          
          flock.push(new Boid(x, y, f));
        }
    ...
    const updateFormations = () => {
      ...
      // Form letters every 3-5 seconds (180-300 frames at 60fps)
      const letterInterval = 180 + Math.floor(Math.random() * 120);
      
      if (letterTimer > letterInterval) {
        letterTimer = 0;
        
        // Choose a random flock to form a letter
        const flockIndex = Math.floor(Math.random() * flocks.length);
        const flock = flocks[flockIndex];
        
        // Alternate between M and G
        const letters = ['M', 'G'];
        const letter = letters[currentLetterIndex % 2];
        currentLetterIndex++;
        
        // Random position for the letter
        const canvasWidth = canvas?.width || 0;
        const canvasHeight = canvas?.height || 0;
        const centerX = canvasWidth * (0.25 + Math.random() * 0.5);
        const centerY = canvasHeight * (0.25 + Math.random() * 0.5);
        
        // Assign letter formation targets to this flock
        flock.targetFormation = letter;
        flock.formationProgress = 0;
        
        flock.birds.forEach((bird, index) => {
          if (letter === 'M') {
            bird.targetPosition = getLetterMPosition(index, flock.birds.length, centerX, centerY);
          } else {
            bird.targetPosition = getLetterGPosition(index, flock.birds.length, centerX, centerY);
          }
        });
        
        // Clear formation after 2-3 seconds
        setTimeout(() => {
          flock.targetFormation = null;
    ...
    const animate = () => {
      ...
      ctx!.fillStyle = 'rgba(11, 21, 38, 0.06)';
      ctx!.fillRect(0, 0, canvasWidth, canvasHeight);
      
      updateFormations();
      
      // Collect all birds for interaction
      const allBirds: Boid[] = [];
      flocks.forEach(f => allBirds.push(...f.birds));
      
      // Update and draw all flocks
      flocks.forEach(flock => {
        // Update formation progress
        if (flock.targetFormation) {
          flock.formationProgress = Math.min(1, flock.formationProgress + 0.02);
        } else {
          flock.formationProgress = Math.max(0, flock.formationProgress - 0.03);
        }
        
        // Update each bird
        flock.birds.forEach(bird => {
          bird.edges();
          bird.flock(allBirds, flock.formationProgress);
          bird.update();
          bird.draw();
        });
      });
