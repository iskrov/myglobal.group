  ...
  useEffect(() => {
    ...
    class Bird {
      ...
      constructor(x: number, y: number, z: number = 0) {
        ...
        this.maxSpeed = 3;
        this.maxForce = 0.15;
        this.baseSize = 8 + Math.random() * 4; // SMALLER: 8-12px instead of 15-25px
        this.alpha = 0.9;
      ...
      getApparentSize(): number {
        ...
        const depthScale = 0.5 + this.depth * 1.0; // Scale from 0.5x to 1.5x (reduced from 0.3x to 2x)
      ...
      flock(birds: Bird[], formationStrength: number): void {
        ...
        if (formationStrength > 0.5) {
          ...
          cohesion.mult(0.3);
          separation.mult(0.8);
          target.mult(5.0 * formationStrength); // Much stronger target seeking
        } else {
          // Natural flocking with depth variation
          alignment.mult(1.5);
          cohesion.mult(1.0);
          separation.mult(2.0);
          target.mult(0.2);
          
          // Add some Z-axis movement for depth during flow
      ...
      draw(ctx: CanvasRenderingContext2D): void {
        ...
        // Simple V-shaped bird like in the reference image
        const wingSpan = size;
        const wingAngle = 0.3 + Math.sin(this.wingPhase) * 0.1; // Subtle wing movement
        
        // RESTORE BLUE COLOR with depth variation
        const depthBrightness = 0.6 + this.depth * 0.4; // 0.6 to 1.0
        ctx.strokeStyle = `rgba(${47 * depthBrightness}, ${127 * depthBrightness}, 255, ${alpha})`;
    ...
    // Initialize birds with varied depths
    const birds: Bird[] = [];
    const numberOfBirds = 250 + Math.floor(Math.random() * 100); // 250-350 birds for clearer letters

    // Letter formation positions (simplified, no depth variation for clarity)
    const getLetterMPosition = (index: number, total: number): Vector => {
      const centerX = canvasWidth * 0.25;
      const centerY = canvasHeight * 0.5;
      const letterHeight = 100; // Smaller for better fit
    ...
    const getLetterGPosition = (index: number, total: number): Vector => {
      ...
      const centerY = canvasHeight * 0.5;
      const radius = 50; // Smaller for better fit
      const strokeWidth = 12;
    ...
    // Initialize birds in 3D space
    const init = () => {
      birds.length = 0;

      // Create birds at various depths
      for (let i = 0; i < numberOfBirds; i++) {
        const angle = (i / numberOfBirds) * Math.PI * 2;
    ...
    // Phase management with depth effects
    let currentPhase: 'flow' | 'forming' | 'holding' | 'dispersing' = 'flow';
    let phaseTimer = 0;
    ...
    const updatePhase = (time: number) => {
      ...
      // Speed up first formation
      const flowDuration = time < 200 ? 100 : 300 + Math.random() * 120;
      ...
      switch(currentPhase) {
        case 'flow':
          if (phaseTimer > flowDuration) {
            ...
            // Assign birds to form M and G at different depths
            const halfBirds = Math.floor(birds.length / 2);
            birds.forEach((bird, i) => {
              if (i < halfBirds) {
                bird.letterType = 'M';
                bird.targetPosition = getLetterMPosition(i, halfBirds);
              } else {
                bird.letterType = 'G';
                bird.targetPosition = getLetterGPosition(i - halfBirds, birds.length - halfBirds);
              }
            });
          }
        ...
        case 'forming':
          formationStrength = Math.min(1, formationStrength + 0.02); // Slower increase for smoother formation
          if (phaseTimer > 120) { // More time to form letters
        ...
        case 'holding':
          if (phaseTimer > 90) { // Hold letters longer for visibility
