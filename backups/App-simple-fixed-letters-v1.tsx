...
// DENSE MURMURATION WITH RELIABLE LETTER FORMATIONS
const FlockAnimation: React.FC = () => {
  ...
  useEffect(() => {
    ...
    let birds: Bird[] = [];
    let time = 0;
    let currentPhase = 'flow'; // 'flow', 'M', 'G'
    let phaseTimer = 0;
    ...
    class Bird {
      ...
      constructor(x: number, y: number) {
        ...
        // Visual properties - small for density
        this.size = 1.2 + Math.random() * 0.6; // 1.2-1.8px
      ...
      seekTarget(): Vector {
        ...
        steer.limit(this.maxForce * 2); // Strong force for letter formation
        return steer;
      }
      ...
      flock(birds: Bird[]): void {
        ...
        // Adjust weights based on whether we're forming letters
        if (formationStrength > 0) {
          alignment.mult(0.5);
          cohesion.mult(0.8);
          separation.mult(1.5);
          target.mult(3 * formationStrength); // Strong when forming
        } else {
          alignment.mult(1.2);
          cohesion.mult(1.5);
          separation.mult(2.0);
      ...
      draw(): void {
        ...
        if (this.isLeader) {
          ...
          ctx!.beginPath();
          ctx!.arc(this.position.x, this.position.y, this.size * 1.3, 0, Math.PI * 2);
          ctx!.fill();
        } else {
          ctx!.fillStyle = 'rgba(47, 107, 255, 0.85)';
          ctx!.beginPath();
    ...
    // Get M formation position - FIXED POSITION ON LEFT
    const getLetterMPosition = (index: number, total: number): Vector => {
      const canvasWidth = canvas?.width || 0;
      const canvasHeight = canvas?.height || 0;
      const centerX = canvasWidth * 0.3; // Fixed left position
    ...
    // Get G formation position - FIXED POSITION ON RIGHT
    const getLetterGPosition = (index: number, total: number): Vector => {
      const canvasWidth = canvas?.width || 0;
      const canvasHeight = canvas?.height || 0;
      const centerX = canvasWidth * 0.7; // Fixed right position
      const centerY = canvasHeight * 0.5;
      const radius = 60;
      ...
      if (index < arcBirdCount) {
        ...
        // Horizontal bar
        const barProgress = (index - arcBirdCount) / Math.max(1, total - arcBirdCount - 1);
        return new Vector(
          centerX + barProgress * radius * 0.7,
          centerY
        );
      }
    };
    ...
    const init = () => {
      ...
      // Dense flock - 500-700 birds
      const numberOfBirds = Math.min(700, Math.max(500, Math.floor(canvasWidth / 2.5)));
      
      console.log(`Creating murmuration with ${numberOfBirds} birds`);
      
      // Start with some clustering for natural flock appearance
      const clusters = 5;
      for (let c = 0; c < clusters; c++) {
        const clusterX = Math.random() * canvasWidth;
        const clusterY = Math.random() * canvasHeight;
        const birdsInCluster = Math.floor(numberOfBirds / clusters);
        
        for (let i = 0; i < birdsInCluster; i++) {
    ...
    const updatePhase = () => {
      ...
      // Phase timing: flow (5-7 sec) -> M (2-3 sec) -> flow (5-7 sec) -> G (2-3 sec) -> repeat
      if (currentPhase === 'flow' && phaseTimer > 300 + Math.random() * 120) { // 5-7 seconds of flow
        // Switch to M or G alternately
        currentPhase = (time % 2000 < 1000) ? 'M' : 'G';
        phaseTimer = 0;
        
        // Assign targets to all birds
        birds.forEach((bird, index) => {
          if (currentPhase === 'M') {
            bird.targetPosition = getLetterMPosition(index, birds.length);
          } else {
            bird.targetPosition = getLetterGPosition(index, birds.length);
          }
        });
      } else if ((currentPhase === 'M' || currentPhase === 'G') && phaseTimer > 120 + Math.random() * 60) { // 2-3 seconds of letter
        // Back to flow
        currentPhase = 'flow';
        phaseTimer = 0;
      ...
      if (currentPhase === 'M' || currentPhase === 'G') {
