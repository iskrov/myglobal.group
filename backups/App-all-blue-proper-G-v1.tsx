...
const FlockAnimation: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const getLetterG1Position = (index: number, total: number): Vector => {
      ...
      const mainArcBirds = Math.floor(total * 0.65); // 65% for C-arc
      const horizontalBarBirds = Math.floor(total * 0.20); // 20% for horizontal bar
      const verticalLineBirds = total - mainArcBirds - horizontalBarBirds; // 15% for vertical line
      ...
      if (index < mainArcBirds) {
        ...
        // Horizontal bar extending into the C
        const t = (index - mainArcBirds) / horizontalBarBirds;
        x = centerX - radius * 0.2 + t * radius * 0.7; // Bar goes from inside C to right
        y = centerY;
      } else {
        // Vertical line going DOWN from the right end of the bar
        const t = (index - mainArcBirds - horizontalBarBirds) / verticalLineBirds;
        x = centerX + radius * 0.5; // Right side of the bar
        y = centerY + t * radius * 0.6; // Going down
      }
    ...
    const updatePhase = (time: number) => {
      ...
      switch(currentPhase) {
        case 'murmuration':
          // 8 seconds of murmuration (480 frames at 60fps)
          if (phaseTimer > 480) {
