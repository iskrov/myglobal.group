  ...
  useEffect(() => {
    ...
    class Bird {
      ...
      constructor(x: number, y: number, z: number = 0) {
        ...
        this.maxSpeed = 3;
        this.maxForce = 0.15;
        this.baseSize = 15 + Math.random() * 10; // Varying base sizes like in the image
      ...
      getApparentSize(): number {
        ...
        const depthScale = 0.3 + this.depth * 1.7; // Scale from 0.3x to 2x
      ...
      flock(birds: Bird[], formationStrength: number): void {
        ...
        if (formationStrength > 0.5) {
          ...
          cohesion.mult(0.5);
          separation.mult(1.0);
          target.mult(3.0 * formationStrength);
        } else {
      ...
      draw(ctx: CanvasRenderingContext2D): void {
        ...
        const wingSpan = size;
        const wingAngle = 0.3 + Math.sin(this.wingPhase) * 0.15; // Subtle wing movement

        // Color gets lighter when closer (depth effect)
        const brightness = 100 + this.depth * 155; // 100-255
        const blue = 200 + this.depth * 55; // 200-255
        ctx.strokeStyle = `rgba(${brightness}, ${brightness * 0.7}, ${blue}, ${alpha})`;
    ...
    const birds: Bird[] = [];
    const numberOfBirds = 150 + Math.floor(Math.random() * 100); // 150-250 birds

    // Letter formation positions (now with depth)
    ...
    const getLetterGPosition = (index: number, total: number): Vector => {
      ...
      if (index < mainArcBirds) {
        ...
        z = Math.sin(t * Math.PI) * 0.5; // Depth variation along arc
